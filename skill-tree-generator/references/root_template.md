...
生成后必须根据实际 skill 能力逐项填写。不要保留空泛占位符。

| 高频共享词 | 无上下文时 | 出现 Skill 名称 / 唯一领域词时 | 多意图并存时 |
|-----------|-----------|-------------------------------|-------------|
| create / 新建 / 生成 | 询问用户选择具体 Skill 或对象类型 | 进入对应 Skill/能力路径 | 保留每个明确命中的路径 |
| export / 导出 / 渲染 | 询问导出对象和目标 Skill | 进入对应 Skill/能力路径 | 保留 shared/export 与专属能力路径 |
| list / info / 查询 / 查看 | 询问查询对象 | 进入对应 Skill/能力路径 | 按子任务分别路由 |
| config / 配置 / 设置 | 询问配置对象和作用域 | 进入对应 Skill/能力路径 | 保留多个配置子任务 |
| test / 测试 / 验证 | 询问测试对象和测试类型 | 进入对应 Skill/能力路径 | 保留每个测试/验证子任务 |

**规则**: 高频共享词只能作为 P3 弱信号。若同一子任务同时出现 P1/P2 信号，P1/P2 覆盖 P3；若当前 prompt 包含多个独立子任务，必须先拆分子任务，再分别应用优先级，不能因为某个子任务命中高优先级路径而丢弃其他明确路径。

## 路由精修证据

`GENERATION-REPORT.md` 必须记录 Routing Refinement Stage 结果：覆盖的 sibling group、发现的 ambiguous signals、修复内容和复验结论。
