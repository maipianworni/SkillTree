...
6. **Self-Containment [MANDATORY]**: The generated skill tree must be fully self-contained and independent from the source skill package or any path outside the tree. Prefer inlining content into leaf nodes. For large file sets handled by `references/error_handling.md` Step R2, a leaf may reference files copied into the same tree by tree-internal relative paths. Never create stub files that only contain a summary and a pointer to an external file. An agent loading only the tree must have everything needed to execute. This includes workflow steps, code examples, API specifications, lookup tables, and any data the original skill contained.
